from Schiffe_versenken_final import Game                                                                                #the method game is imported from the file schiffe versenken final
...
headline = pygame.font.SysFont("futura", 60)                                                                            #setting and saving the font for headlines in a variable
Playerturntext = pygame.font.SysFont("futura", 40)                                                                      #setting and saving another font plus size of the font in a var
continuequit = pygame.font.SysFont("futura", 30)                                                                        #setting and saving yet another font plus size of the font in a var
...
Width = Square * 10 * 2 + Square                                                                                        #the width will be wider than twice the grid (being 10 squares wide), so that there is still some distance between the grids and both grids are fully visable
Height = Square * 10 * 2 + Square                                                                                       #the height will be higher than twice the grid (being 10 squares high), so that there is still some distance between the grids and both grids are fully visable
...
def drawships(player, left=0, top=0):                                                                                   #this is a function that draws the players ships on the grid
    for ship in player.ships:                                                                                           #every ship of the player is called
        ...
        if ship.orientation == "Horizontal":                                                                            #for the horizontal orientation:
            width = ship.size * Square - 2 * SmallerShips                                                               #the width of the ship is indented by twice the INDENT to once cover the indet above and to indet this side of the ship too
            height = Square - 2 * SmallerShips                                                                          #the height is equal the Square Size minus again twice the indet (same reason as with the width)
...
def switchplayer():                                                                                                     #the switchplayer fuction starts here
    ...
    while playerswitch:                                                                                                 #as long playerswitch is true enter while loop
        ...
        if game.player1turn:                                                                                            #when player 1 is the active player
            ...
            textboxplayer1turn = headline.render(textplayer1turn, False, Green, Black)                                  #the str stored in var textplayer1turn is renderd and displayed
            Window.blit(textboxplayer1turn, (Width // 2 - 300, Height // 2 - 50))                                       #the location is set
            textboxmissed = headline.render(textmissed, False, Red)                                                     #the str stored in var textboxmissed is renderd and displayed
            Window.blit(textboxmissed, (Width // 2 - 150, Height // 2 - 200))                                           #the location is set

            textnextplayercq = "Press C to continue or Q to quit"                                                       #text is stored in a variable
            textboxcontinue = continuequit.render(textnextplayercq, False, Black, Green)                                #the str stored in var textboxcontinue is renderd and displayed
            Window.blit(textboxcontinue, (Width // 2 - 200, Height // 2 + 50))                                          #the location is set
        ...
        if not game.player1turn:                                                                                        #when player 2 is the active player
            ...
            textboxplayer2turn = headline.render(textplayer2turn, False, Green, Black)                                  #the str stored in var textplayer2turn is renderd and displayed
            Window.blit(textboxplayer2turn, (Width // 2 - 300, Height // 2 - 50))                                       #the location is set
            textboxmissed = headline.render(textmissed, False, Red)                                                     #the str stored in var textboxmissed is renderd and displayed
            Window.blit(textboxmissed, (Width // 2 - 150, Height // 2 - 200))                                           #the location is set

            textpausecontinue = "Press C to continue or Q to quit"                                                      #text is stored in a variable
            textboxcontinue = continuequit.render(textpausecontinue, False, Black, Green)                               #the str stored in var textboxcontinue is renderd and displayed
            Window.blit(textboxcontinue, (Width // 2 - 200, Height // 2 + 50))                                          #the location is set
            pygame.display.update()                                                                                     #display is updated by pygame
    ...
    for event in pygame.event.get():                                                                                    #in case an user event is accuring capture that event
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:                                                                        #if the event is a click on the mouse
            ...
            if not game.over and game.player1turn and x < 10 * Square and y < 10 * Square:                              #as long the game is not over and player one is the active player and the click was within the grid in the top left corner...
                ...
                if not game.player1turn:                                                                                #when player one missed...
                ...
                index = row * 10 + col                                                                                  #...this is now stored as the actual index which represents the grid, we are working with in this programm
                game.playerturn(index)                                                                                  #the position is now stored to the players index
                if game.player1turn:                                                                                    #when player two missed...
                    switchplayer()                                                                                      #...the switchplayer function is called
        ...
        if animating:                                                                                                   #in case an event accured prior that set animating to false, the loop stops here and the pygame window closes
            ...
            if game.player1turn:                                                                                        #in case player one is the active player the following happens
                ...
                textbox = Playerturntext.render(textplayer1turn, False, Green, Black)                                   #the str stored in var textplayer1turn is renderd and displayed
                Window.blit(textbox, (Width // 2 - 50, Height // 2 - 300))                                              #the location is set
                textyourships = "Your Ships ->"                                                                         #text is stored in var
                textbox = Playerturntext.render(textyourships, False, Green, Black)                                     #the str stored in var textyourships is renderd and displayed
                Window.blit(textbox, (Width // 2 - 200, Height // 2 + 150))                                             #the location is set
                ...
                textbox = Playerturntext.render(textplayer2turn, False, Green, Black)                                   #the str stored in var textplayer2turn is renderd and displayed
                Window.blit(textbox, (Width // 2 - 350, Height // 2 + 150))                                             #the location is set
                textyourships = "<- Your Ships"                                                                         #text is stored in var
                textbox = Playerturntext.render(textyourships, False, Green, Black)                                     #the str stored in var textyourships is renderd and displayed
                Window.blit(textbox, (Width // 2 - 50, Height // 2 - 300))                                              #the location is set
            ...
            if game.over:                                                                                               #in the case game.over is set true the following is happening
                ...
                Window.blit(textbox, (Width // 2 - 200, Height // 2 - 50))                                              #the location is set

                textstartagain = "Press Enter to start again"                                                           #text is stored in var
                textboxstartagain = continuequit.render(textstartagain, False, Green, Black)                            #the str stored in var textstartagain is renderd and displayed
                Window.blit(textboxstartagain, (Width // 2 - 190, Height // 2 + 50))                                    #the location is set
                pygame.display.update()                                                                                 #display is updated

            pygame.display.flip()                                                                                       #pygame display update function
